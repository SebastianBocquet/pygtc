...
@image_comparison(baseline_images=['img'], extensions=['png'])
def test_img():
    fig = plt.figure()
...
@image_comparison(baseline_images=['bare'], extensions=['png'])
def test_GTC_bare():
...
@image_comparison(baseline_images=['truthLabels_withTex'], extensions=['png'])
...
@image_comparison(baseline_images=['truthLabels_truthLineStyles'], extensions=['png'])
...
@image_comparison(baseline_images=['nConfidenceLevels'], extensions=['png'])
def test_GTC_nConfidenceLevels():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            nConfidenceLevels = 3)

@image_comparison(baseline_images=['gaussianConfLevels'], extensions=['png'])
def test_GTC_gaussianConfLevels():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            gaussianConfLevels = True)

@image_comparison(baseline_images=['nBins'], extensions=['png'])
def test_GTC_nBins():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            nBins = 20)

@image_comparison(baseline_images=['smoothingKernel'], extensions=['png'])
def test_GTC_smoothingKernel():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            smoothingKernel = 2)

@image_comparison(baseline_images=['filledPlots'], extensions=['png'])
def test_GTC_filledPlots():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            filledPlots = False)

@image_comparison(baseline_images=['plotDensity'], extensions=['png'])
def test_GTC_plotDensity():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            plotDensity = True)

@image_comparison(baseline_images=['figureSize'], extensions=['png'])
def test_GTC_figureSize():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            figureSize = 'APJ_page')

@image_comparison(baseline_images=['panelSpacing'], extensions=['png'])
...
@image_comparison(baseline_images=['labelRotation'], extensions=['png'])
def test_GTC_labelRotation():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            labelRotation = (False, False))

@image_comparison(baseline_images=['tickShifts'], extensions=['png'])
def test_GTC_tickShifts():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            tickShifts = (0.2, 0.2))

@image_comparison(baseline_images=['colorsOrder'], extensions=['png'])
def test_GTC_colorsOrder():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            colorsOrder = ['purples', 'yellows'])

@image_comparison(baseline_images=['do1dPlots'], extensions=['png'])
def test_GTC_do1dPlots():
    pygtc.plotGTC(chains=[SAMPLES_1,SAMPLES_2],
                            do1dPlots = False)

@image_comparison(baseline_images=['doOnly1dPlot'], extensions=['png'])
def test_GTC_doOnly1dPlot():
    input_chains = [np.array([SAMPLES_1[:,0]]).T, np.array([SAMPLES_2[:,0]]).T]
    pygtc.plotGTC(chains=input_chains,
                            doOnly1dPlot = True)

@image_comparison(baseline_images=['mathTextFontSet'], extensions=['png'])
